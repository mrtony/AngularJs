...
(function () {
    ...
    Component2.$inject = ['toastr']; 

    function Component2(toastr) {
        ...
        //Column Definition : http://www.ag-grid.com/angular-grid-column-definitions/index.php
        //headerName: 顯示的欄位名稱
        //field: 要bind到rowData中的key名稱
        //cellRenderer: 
        //suppressSorting: 在enableSorting=true時, 關閉(true)或打開(false)此欄位sorting的功能
        //width: 欄寬
        //filter: 
        var columnDefs = [
            {headerName: '', width: 30, checkboxSelection: true, suppressSorting: true, suppressMenu: true},
            {headerName: "Name", field: "name", headerGroup: 'Employee', width: 150, cellRenderer: timestampCellRenderer},
        ...
        //資料載入完成後會呼叫的callback, 取得載入的資料總數
        function onModelUpdated() {
            var model = vm.gridOptions.api.getModel();
            var totalRows = vm.gridOptions.rowData.length;
            var processedRows = model.getVirtualRowCount();
            vm.rowCount = processedRows.toLocaleString() + ' / ' + totalRows.toLocaleString();
        }
        ...
        function createAdvancedGrid() {
            //vm.gridOptions = basicGridOption;
        }
        
        //執行後會更新內部的資料結構,但並不會修改任何的內容. 比如說改了rowData的內容後去呼叫refreshView(), grid內容還是不變
